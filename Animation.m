...
SimTheta1 = out.Theta.Data(:,1)';
SimTheta2 = out.Theta.Data(:,2)';
SimTheta3 = out.Theta.Data(:,3)';

Simx1 = -sin(SimTheta1);
Simy1 = cos(SimTheta1);
Simz1 = 0.*(SimTheta1);

Simx2 = -sin(SimTheta1)+(cos(SimTheta1).*sin(SimTheta2));
Simy2 = cos(SimTheta1)+(sin(SimTheta1).*sin(SimTheta2));
Simz2 = -cos(SimTheta2);

Simx3 = -sin(SimTheta1)+(cos(SimTheta1).*(sin(SimTheta2) + sin(SimTheta3) ));
Simy3 = cos(SimTheta1)+(sin(SimTheta1).*(sin(SimTheta2) + sin(SimTheta3) ));
Simz3 = -cos(SimTheta2)-(cos(SimTheta3));

v=VideoWriter("Week22Animation2");
open(v);
...
for x=1:length(Time)
    ...
    title("Time: "+num2str(Time(x),formatSpec));
